...
const directUpiBtn = document.getElementById('direct-upi-btn');
...
const openCoffeeModal = () => {
    ...
    if (coffeeModal) coffeeModal.classList.add('active');
};
const closeCoffeeModalFn = () => {
    if (coffeeModal) coffeeModal.classList.remove('active');
};

if (coffeeBtn) coffeeBtn.onclick = openCoffeeModal;
if (closeCoffeeModal) closeCoffeeModal.onclick = closeCoffeeModalFn;

let currentSelectedAmount = '50';

function getUpiUri(scheme, amount) {
    const baseParams = `pa=nsb566@oksbi&pn=BunkerMate&am=${amount}&cu=INR&tn=Support%20BunkerMate`;
    switch (scheme) {
        case 'gpay':
...
function launchUpi(scheme) {
    const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
    const uri = getUpiUri(scheme, currentSelectedAmount);

    if (!isMobile) {
        showToast('On PC: Scan QR with your phone app or copy UPI ID ☕');
    }

    // Trigger redirection to selected UPI app or system app chooser
...
function initCoffeeModal() {
    ...
    amountChips.forEach(chip => {
        chip.onclick = () => {
            ...
            currentSelectedAmount = chip.dataset.amount || '50';
        };
    });

    // Individual UPI app buttons (GPay, PhonePe, Paytm, BHIM)
    document.querySelectorAll('.upi-app-btn').forEach(btn => {
        btn.onclick = (e) => {
            e.preventDefault();
            const scheme = btn.dataset.scheme || 'upi';
            launchUpi(scheme);
        };
    });

    // Universal "Pay via Any UPI App" button
    if (directUpiBtn) {
        directUpiBtn.onclick = (e) => {
            e.preventDefault();
            launchUpi('upi');
        };
    }
